...
class GetPhotoHome(BaseView):
	template_name = 'lshare/photo.html'
	template_name_m = 'lshare/photo_m.html'
	rows_per_page = 10
	page_size = 24
	section_size = 240
	...
	def get(self, request):
		...
		if on_tops.exists():
			...
			top_photo.image_url = settings.MEDIA_URL + settings.PHOTO_ROOT + '/'.join(['s950', os.path.basename(top_photo.image.name)])
